# -*- coding: utf-8 -*-
"""
Created on Tue May 17 17:42:45 2022
...
class Planet:
    ...
    def __init__(self, x, y, radius, color, mass, name="Planet"):
        self.x = x
        self.y = y
        self.radius = radius
        self.color = color
    ...
    def draw(self, win):
        ...
        pygame.draw.circle(win,self.color,(x,y), self.radius*0.5)
        if not self.sun:
            distance_text = FONT.render(f"{round(self.distance_to_sun/self.AU, 1)}AU", 1, WHITE)
            win.blit(distance_text, (x - distance_text.get_width()/2, y - distance_text.get_width()/2))
            name_text = FONT.render(f"{self.name}", 1, WHITE)
            WIN.blit(name_text, (x - distance_text.get_width()/2, y - distance_text.get_width()/1))
